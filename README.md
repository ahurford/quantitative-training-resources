...
5. Open a web browser and go to the Github repository online. Log on to Github. You will need to fork the repository, i.e. create your own `branch`. A  `branch` is your personal copy of the repository. You are creating a `branch` because you do not have permission to change the `master` (the main) branch of the repository. Click on the grey boutton `branch: master` (near top left). Enter a name for you branch and create it.
